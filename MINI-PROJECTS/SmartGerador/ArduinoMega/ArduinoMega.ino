...
void setup() {
  ...
  emonRede.voltage(PIN_VOLTAGE_REDE, 217.5, 1.7);
  //Current: input pin, calibration..
  emonRede.current(PIN_CURRENT, 10.9);

  //Voltage:input pin, calibration, phase_shift
  emonGerador.voltage(PIN_VOLTAGE_GERADOR, 312, 1.7);
...
void loop() {
  ...
  if (!digitalRead(BTN_MODO)) {
    while(!digitalRead(BTN_MODO)) {
      printModoLCD(0);
      delay(2000);
    }
    
    statusModo = (statusModo == 'M')? 'A': 'M'; 
    printModoLCD(2);
    delay(2000);
  }
...
void printModoLCD(byte num) {
  ...
  if(num==2)
  {
    ...
    lcd.print("#      PARA    #");
    lcd.setCursor(0, 3);
    String modo = (statusModo == 'M') ? "  MANUAL " : "AUTOMATICO";
    lcd.print(modo);
  }
}
